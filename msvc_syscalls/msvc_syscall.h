...
namespace jm {
    ...
    namespace detail {
        ...
        public:
            ...
            exports_directory(const char* base) noexcept
                : _base(base)
            {
            ...
            const char* address(size_type index) const
                noexcept
            {
    ...
    std::uint32_t hash(const char* str) noexcept
    {
    ...
    std::map<std::uint32_t, std::int32_t> id_list;
    ...
    void init_syscalls_list()
    {
        detail::exports_directory exports(static_cast<const char*>(detail::ntdll_base()));
        for (auto i = exports.size() - 1;; --i) {
            if (exports.name(i)[0] == 'N') {
                id_list.insert({ hash(exports.name(i)), *reinterpret_cast<const std::int32_t*>(exports.address(i) + 4) });
            }
            if (i == 0)
                break;
        }
    ...
    template<typename T>
    auto manual_syscall(std::int32_t id) {
        set_id(id);
        T* h_pointer = reinterpret_cast<T*>(syscall_handle);
        return h_pointer;
    }
...
#define SYSCALL(function_name)  \
    jm::manual_syscall<decltype(function_name)>(jm::id_list[jm::hash(#function_name)])
